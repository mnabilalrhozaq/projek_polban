    ...
    <div class="main-content">
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <!-- Pagination -->
                <?php if (!empty($users) && isset($pager) && $pager->getPageCount() > 1): ?>
                <div class="mt-4">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <div class="text-muted">
                            <small>
                                Menampilkan 
                                <strong><?= count($users) ?></strong> dari 
                                <strong><?= $stats['total'] ?? 0 ?></strong> total users
                            </small>
                        </div>
                        <div class="text-muted">
                            <small>
                    ...
                    $filterParam = '';
                    if ($roleFilter) $filterParam .= '&role=' . $roleFilter;
                    if ($statusFilter !== '') $filterParam .= '&status=' . $statusFilter;
                    if ($unitFilter) $filterParam .= '&unit=' . $unitFilter;
                    ?>
                    <nav aria-label="Pagination">
                        <ul class="pagination justify-content-center">
                            ...
                            <?php if ($pager->getCurrentPage() > 1): ?>
                                <li class="page-item">
                                    <a class="page-link" href="<?= base_url('/admin-pusat/user-management?page=' . ($pager->getCurrentPage() - 1) . $filterParam) ?>" aria-label="Previous">
                            ...
                            <?php for ($i = 1; $i <= $pager->getPageCount(); $i++): ?>
                                ...
                                <?php else: ?>
                                    <li class="page-item">
                                        <a class="page-link" href="<?= base_url('/admin-pusat/user-management?page=' . $i . $filterParam) ?>"><?= $i ?></a>
                                    </li>
                                <?php endif; ?>
                            <?php endfor; ?>
                            ...
                            <?php if ($pager->getCurrentPage() < $pager->getPageCount()): ?>
                                <li class="page-item">
                                    <a class="page-link" href="<?= base_url('/admin-pusat/user-management?page=' . ($pager->getCurrentPage() + 1) . $filterParam) ?>" aria-label="Next">
